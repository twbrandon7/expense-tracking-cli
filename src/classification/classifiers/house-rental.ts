...
export interface HouseRentalOptions {
  description: string;
  counterparty: string;
  is_regex?: boolean;
}

export class HouseRentalClassifier implements TransactionClassifier<HouseRentalOptions> {
  readonly id = 'house_rental';

  match(row: TransactionRow, options: HouseRentalOptions): boolean {
